...
describe("Card", () => {
  ...
  it("should throw an error for an invalid card number", () => {
    expect(() => new Card({ card: -1 })).toThrow("Invalid card");
    expect(() => new Card({ card: 53 })).toThrow("Invalid card");
  });
  ...
  it("should assign correct points to cards and update Ace value", () => {
    const aceCard = new Card({ card: Suit.Clubs + Rank.Ace });
    const tenCard = new Card({ card: Suit.Clubs + Rank.Ten });
  ...
  it("should throw an error when setting Ace points on a non-Ace card", () => {
    const card = new Card({ card: Suit.Clubs + Rank.Ten });
